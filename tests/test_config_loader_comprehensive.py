...
class TestConfigLoaderComprehensive(unittest.TestCase):
    ...
    def test_get_install_directory(self):
        """Test that _get_install_directory returns correct path."""
        install_dir = _get_install_directory()
        self.assertIsInstance(install_dir, Path)
        self.assertTrue(install_dir.exists())

    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_file_not_found(self, mock_get_install_dir):
        """Test ConfigError when config file doesn't exist."""
        mock_install_dir = MagicMock()
        mock_config_path = MagicMock()
        mock_config_path.exists.return_value = False
        mock_install_dir.__truediv__.return_value = mock_config_path
    ...
    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_invalid_json(self, mock_get_install_dir):
    ...
    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_missing_required_keys(self, mock_get_install_dir):
    ...
    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_invalid_types(self, mock_get_install_dir):
    ...
    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_success_with_optional_fields(self, mock_get_install_dir):
    ...
    @patch("vc_commit_helper.config.loader._get_install_directory")
    def test_load_config_os_error(self, mock_get_install_dir):
