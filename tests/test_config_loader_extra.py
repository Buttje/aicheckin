...
class TestConfigLoaderExtra(unittest.TestCase):
    ...
    def _write_config(self, install_dir: Path, data: dict) -> Path:
        path = install_dir / ".ollama_config.json"
    ...
    def test_invalid_base_url_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_config(install_dir, {"base_url": 123, "port": 1, "model": "m"})
    ...
    def test_invalid_port_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            install_dir = Path(tmp)
            self._write_config(install_dir, {"base_url": "http://", "port": "not_int", "model": "m"})
            with patch('vc_commit_helper.config.loader._get_install_directory', return_value=install_dir):
                with self.assertRaises(ConfigError) as cm:
                    load_config()
    ...
    def test_invalid_model_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            install_dir = Path(tmp)
            self._write_config(install_dir, {"base_url": "http://", "port": 1, "model": 123})
            with patch('vc_commit_helper.config.loader._get_install_directory', return_value=install_dir):
                with self.assertRaises(ConfigError) as cm:
                    load_config()
    ...
    def test_invalid_request_timeout_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            install_dir = Path(tmp)
            self._write_config(install_dir, {"base_url": "http://", "port": 1, "model": "m", "request_timeout": "not_number"})
            with patch('vc_commit_helper.config.loader._get_install_directory', return_value=install_dir):
                with self.assertRaises(ConfigError) as cm:
                    load_config()
    ...
    def test_invalid_max_tokens_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            install_dir = Path(tmp)
            self._write_config(install_dir, {"base_url": "http://", "port": 1, "model": "m", "max_tokens": 3.14})
            with patch('vc_commit_helper.config.loader._get_install_directory', return_value=install_dir):
                with self.assertRaises(ConfigError) as cm:
                    load_config()
    ...
    def test_valid_optional_fields(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_config(install_dir, config)
            with patch('vc_commit_helper.config.loader._get_install_directory', return_value=install_dir):
                result = load_config()
